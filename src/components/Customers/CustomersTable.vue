<template>
  <UiParentCard title="Listado de Clientes">
    <template v-slot:action>
        <div class="d-flex align-center gap-3">
            <ExpandableSearch 
              v-model="search" 
              label="Nombre, dni, correo, teléfono..." 
            />
        </div>
    </template>
        ...
        <tbody>
          <template v-if="paginatedRecords.length > 0">
            <tr v-for="record in paginatedRecords" :key="record.id">
              <td>{{ record.id }}</td>
              <td>{{ record.tx_dni_type }}-{{ record.tx_dni }}</td>
              <td>{{ record.tx_first_name }}</td>
              <td>{{ record.tx_last_name }}</td>
              <td>{{ record.tx_phone }}</td>
              <td>{{ record.tx_email }}</td>
              <td>
            ...
            <tr v-else>
                <td colspan="12">
                    ...
                    :description="search 
                        ...
                        : 'Aún no hay nada que mostrar acá. ¡Agrega tu primer cliente!'"
                    >
...
const fetchUsers = async () => {
    try {
        const res = await axios.get(`${API_BASE}api/customers/all`);
...
const filteredrecords = computed(() => {
  ...
  return records.value.filter(v => {
    const target = [
        ...
        v.tx_dni_type?.toString().toLowerCase() ?? '',
